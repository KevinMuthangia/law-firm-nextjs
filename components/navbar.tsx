...
const NavbarLinks = ({closeMenu, title, link, selected}: NavLinksProps) => {
	return	(
		selected 
		? <li onClick={closeMenu} className="hover-text-effect-selected text-xl font-medium"><Link href={link}>{title}</Link></li>
		:  <li onClick={closeMenu} className="hover-text-effect text-xl font-medium"><Link href={link}>{title}</Link></li>
	)
};
